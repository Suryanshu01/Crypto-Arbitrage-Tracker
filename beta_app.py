...
path_array=["Bitvavo", "Huobi", "Coinbase", "Bitfinex", "Binance"]
print(path_array)

def functions(x,y,z):
    if x == "Bitvavo":
        return get_bitvavo_prices(y,z)[3]
    elif x == "Huobi":
        return get_huobi_prices(y,z)[1]
    elif x == "Coinbase":
        return get_coinbase_prices(y,z)[1]
    elif x == "Bitfinex":
        return get_bitfinex_prices(y,z)[1]
    elif x == "Binance":
        return get_binance_prices(y,z)[1]
...
def main():
    ...
    for sCoin in SOLID_COIN:
        for vCoin in VARIABLE_COIN:
            if sCoin != vCoin:
                try:
                 for buy_path in path_array:
                    buy_price=functions(buy_path,vCoin,sCoin)

                    for sell_path in path_array:
                        if sell_path != buy_path:
                            sell_price=functions(sell_path,vCoin,sCoin)


                            print(f"Arbitrage buy Price----> {buy_price} | {vCoin}-{sCoin} | {buy_path}")

                            print(f"Arbitrage sell Price----> {sell_price} | {vCoin}-{sCoin} | {sell_path}")
                            outputper=(sell_price-buy_price)/buy_price*100
                            print(f"profit/loss percentage----> {(sell_price-buy_price)/buy_price*100}")
                            ...
                            if outputper>=0:
                                print(
                                colored(
                                    f"{current_time}\t{sCoin.upper()}\t\t{vCoin.upper()}\t\t{buy_path}\t{sell_path}\t{buy_price}\t{sell_price}",
