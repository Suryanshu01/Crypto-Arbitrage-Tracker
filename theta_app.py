...
path_array=["Dydx", "Uniswap", "Dodo", "Pancakeswap", "Sushiswap"]
print(path_array)
...
def main(*args):
    print(
        colored(
            "DayTime\t\t\t\tSolid coin\t\tVariable coin\t\tExchange\t\tAmount\t\tPrice",
            "white",
            "on_grey",
            ["bold", "underline"],
        )
                ...
                # try:
                    ...
                    for exchange, func in exchange_functions.items():
                        try:
                            ...
                            print(
                                colored(
                                    f"{current_time}\t\t{vCoin.upper()}\t\t{sCoin.upper()}\t\t\t\t{exchange}\t\t\t{amount}\t\t\t\t{prices_dict[exchange]}"))
                        except Exception as e:
                            print(f"Not found for {exchange}")
                    print(colored(f"-" * 200))
