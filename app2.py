...
def get_uniswap_price_v2(contract, token_a,token_b,amount):
    try:    
        decimal_a = get_token_decimal(token=token_a)
        prices = contract.functions.getAmountsOut(int(amount * 10**decimal_a),[token_a, COINS["wbnb"],token_b]).call()
        decimal_b = get_token_decimal(token=token_b)
        return (prices[2] / (10 ** decimal_b), f"{COIN_SYMBOLS[token_a.lower()].upper()}/{COIN_SYMBOLS[token_b.lower()].upper()}")
        
    except Exception as e:
        print(f"and Expectedd error has accured {e}")
        return(0,'NULL')
                ...
                for router_buy in ROUTES:
                    ...
                    for router_sell in ROUTES:
                        if router_buy != router_sell:
                            ...
                            # print("Availavel exchanges--------",ccxt.exchanges)
