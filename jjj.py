...
token_decimals = {
    ...
    "usdc": 6,  # [^5^][13]
    "fxs": 18,  # [^6^][10]
    "mc": 18,  # Assuming MC token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1]
    "frax": 18,  # [^7^][34]
    "dai": 18,  # [^8^][29]
    "woo": 18,  # Assuming WOO token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1]
    "uni": 18,  # Assuming UNI token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1]
    "super": 18,  # Assuming SUPER token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1]
    "wbnb": 18,  # Assuming WBNB token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1]
    "cell": 18,  # Assuming CELL token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1]
    "sand": 18,  # Assuming SAND token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1]
    "shib": 18,  # Assuming SHIB token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    'bel': 18,  # Assuming BEL token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    'axs': 18,  # Assuming AXS token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    'ach': 18,  # Assuming ACH token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "cake": 18,  # Assuming CAKE token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "8pay": 18,  # Assuming 8PAY token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "aave": 18,  # Assuming AAVE token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "ach": 18,  # Assuming ACH token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "ada": 18,  # Assuming ADA token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "adx": 18,  # Assuming ADX token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "alice": 18,  # Assuming ALICE token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "alpa": 18,  # Assuming ALPA token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "alpaca": 18,  # Assuming ALPACA token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "alpha": 18,  # Assuming ALPHA token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "ampl": 18,  # Assuming AMPL token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "ankr": 18,  # Assuming ANKR token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "ankrbnb": 18,  # Assuming ANKRBNB token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "antex": 18,  # Assuming ANTEX token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "anymtlx": 18,  # Assuming ANYMTLX token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "aog": 18,  # Assuming AOG token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "ape": 18,  # Assuming APE token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "apx": 18,  # Assuming APX token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "apys": 18,  # Assuming APYS token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "arena": 18,  # Assuming ARENA token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "arpa": 18,  # Assuming ARPA token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "arv": 18,  # Assuming ARV token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "asr": 18,  # Assuming ASR token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "ata": 18,  # Assuming ATA token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "atm": 18,  # Assuming ATM token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "axl": 18,  # Assuming AXL token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "axs": 18,  # Assuming AXS token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "babycake": 18,  # Assuming BABYCAKE token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "bake": 18,  # Assuming BAKE token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "balbt": 18,  # Assuming BALBT token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "band": 18,  # Assuming BAND token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "bat": 18,  # Assuming BAT token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "bath": 18,  # Assuming BATH token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "bbt": 18,  # Assuming BBT token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "bcfx": 18,  # Assuming BCFX token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1],
    "c98": 18,  # Assuming C98 token follows the standard of 18 decimals as most ERC20 tokens do[^2^][1]
}
...
def init_contract(router):
    ...
    if router == "pancakeswap_v2":
        with open("panRouterV2Abi.json") as f:
            v2_abi = json.load(f)
        contract_address = Web3.to_checksum_address("0x10ED43C718714eb63d5aA57B78B54704E256024E")
       ...
       #      print( type(W3.eth.contract(address=contract_address, abi=v2_abi)))
       #      return W3.eth.contract(address=contract_address, abi=v2_abi)'''
...
def get_pancake_price_v2(contract, token_a, token_b, amount):
    ...
    try:
        ...
        price = contract.functions.getAmountsOut(
            int(amount * 10 ** decimal_a), [token_a, token_b]).call()
        decimal_b = 18
        return (price[2] / (10 ** decimal_b),
                f"{COIN_SYMBOLS[token_a.lower()].upper()}/{COIN_SYMBOLS[token_b.lower()].upper()}")
...
def get_uniswap_price_v2(contract, token_a, token_b, amount):
      ...
      try:
          decimal_a = 18
          decimal_b = 18
          prices=uniswap.get_price_input(token0=token_a, token1=token_b, qty=amount* 10**decimal_a)
...
def execute_query():
    file = open("arbitrage_data.csv", "w")
    file.write(
                ...
                for router_buy in ROUTES:
                    ...
                    for router_sell in ROUTES:
                        if router_buy != router_sell:
                            contract_2 = init_contract(router_sell),
                            ...
                            # Conditional assignment for status
                            status = "positive" if arbitrage > 0 else "negative"
...
if __name__ == "__main__":
    ...
    # get_dodo_price()
    # print(contract.functions.getAmountsOut(int(amount * 10 ** decimal_a),
    #                                               [token_a, COINS["wbnb"], token_b]).call())
    execute_query()