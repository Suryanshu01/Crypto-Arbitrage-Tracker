...
def get_uniswap_price_v2(contract, token_a,token_b,amount):
    try:
        ...
        if not pair_address=="0x0000000000000000000000000000000000000000":
            ...
            decimal_b = 18
            return (prices[1] / (10 ** decimal_b), f"{symbols2[token_a].upper()}/{symbols2[token_b].upper()}")
        else:
            return (9, 9)
                    ...
                    if sCoin != vCoin:
                        for router_buy in ROUTES:
                            ...
                            buy_price, buy_path = get_price(
                                router=router_buy,
                                token_a=tokens2[vCoin.upper()],
                                token_b=tokens2[sCoin.upper()],
                                contract=contract,
                                amount=10000

                            )
                            if buy_price is not 9:
                                ...
                                for router_sell in ROUTES:
                                    if router_buy != router_sell:
                                        ...
                                        sell_price, sell_path = get_price(
                                            router=router_sell,
                                            token_a=tokens2[sCoin.upper()],
                                            token_b=tokens2[vCoin.upper()],
                                            # contract = init_contract(router)
                                            contract=contract,
                                            amount=10000

                                        )
                                        if sell_price is not 9:
